...
def cast_lightning(*args, **kwargs):
    ...
    if target:
        results.append({'consumed': True,
                        'target': target,
                        'message': Message('''Spell strikes the {0}.
                                           Damage is {1}'''.format(target.name,
                                                                   damage))})
...
def cast_fireball(*args, **kwargs):
    ...
    if not tcod.map_is_in_fov(fov_map, target_x, target_y):
        results.append({'consumed': False,
                        'message': Message('''You cannot target a tile
                                           outside you field of view''',
                                           tcod.yellow)})
        return results

    results.append({'consumed': True,
                    'message': Message('''The fireball explodes,
                                       burning everything within {0} tiles!'''.
                                       format(radius),
                                       tcod.orange)})
...
def cast_confuse(*args, **kwargs):
    ...
    for entity in entities:
        if entity.x == target_x and entity.y == target_y and entity.ai:
            ...
            results.append({'consumed': True,
                            'message': Message('''The eyes of the {0}
                                               look vacant, as he starts to
                                               stumble around'''.format(
                                               entity.name),
                                               tcod.light_green)})
            break
